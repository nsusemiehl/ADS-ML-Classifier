...
class ml_doc_classifier:
    ...
    def ads_api_call(self, encoded_query):
        """ Add doc string
        """
    ...
    def download_pdfs(self, source, ids, titles, verbose):
        """ Add doc string
        """
        if verbose:
            print(f"{len(titles)} papers to download.")
        with Bar('Downloading', fill='#', max=len(titles), suffix=f'%(index)d/%(max)d - %(percent).1f%% - %(eta)ds remaining') as bar:
